...
25/07/2024	1.0.0.1		DPR, Skyline	Initial version
****************************************************************************
*/

namespace SLC_AS_DMS_24HR_Variation_VM_Size_1
{
	using System;
	...
	public class Script
	{
		...
		public void RunSafe(IEngine engine)
		{
			...
			var microsoftElements = dms.GetElements().Where(e => e.Protocol.Name.Equals("Microsoft Platform") && e.State == Skyline.DataMiner.Core.DataMinerSystem.Common.ElementState.Active);

			List<TestResult> results = new List<TestResult>();

			foreach (var element in microsoftElements)
			{
				...
				var averageVmSizes24HoursTo48Hours = GetAverageVmSizes(element, processNames, now.AddHours(-48), now.AddHours(-24), engine);
				...
				foreach (var process in processes)
				{
					...
					if (averageVmSizes24HoursTo48Hours.TryGetValue(formattedkey, out _) && averageVmSizes24Hours.TryGetValue(formattedkey, out _))
					{
						...
						{
							ParameterName = "24HR Variation VmSize",
